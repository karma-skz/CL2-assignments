...
spa_texts, por_texts = read_training("Language_datasets/spanish_portugese_train.csv")
test_rows = read_test("Language_datasets/spanish_portugese_test.csv")

lines = [
    "# Results: Spanish vs Portuguese (character n-grams)",
    "",
    f"- Train file: Language_datasets/spanish_portugese_train.csv",
    f"- Test file: Language_datasets/spanish_portugese_test.csv",
    "",
]

for n in (1, 2, 3, 4):
    models = {
        "spa": compute_counts(spa_texts, n),
        "por": compute_counts(por_texts, n),
    }
    ...
    lines += [
        ...
        "| --- | --- | --- | --- |",
        f"| spa | {cls['spa'][0]:.6f} | {cls['spa'][1]:.6f} | {cls['spa'][2]:.6f} |",
        f"| por | {cls['por'][0]:.6f} | {cls['por'][1]:.6f} | {cls['por'][2]:.6f} |",
        "",
    ]

with open("result_por_spa.md", "w", encoding="utf-8") as f:
    f.write("\n".join(lines))
print(f"Wrote report for spanish vs portuguese")
