...
def doc_tokens_with_negation(line):
    """Apply negation feature and return unique tokens (binarization)"""
    ...
    for t in toks:
        ...
        if negating and re.match(r"^\w+$", t):
            ...
            res.append(t)
    return set([x for x in res if re.match(r"^\w+$", x)])
...
def train(train_neg, train_pos):
    ...
    for l in train_neg:
        for w in doc_tokens_with_negation(l):
            neg_doc_counts[w] += 1
    for l in train_pos:
        for w in doc_tokens_with_negation(l):
            pos_doc_counts[w] += 1
...
def predict(text, model):
    words = doc_tokens_with_negation(text)
    vocab = model['vocab']
    
    log_neg = math.log(model['p_neg'])
    log_pos = math.log(model['p_pos'])
    
    # Bernoulli Naive Bayes with negation features
    for w in vocab:
        ...
        if w in words:  # Token is present in document
            log_neg += math.log(p_w_neg)
            log_pos += math.log(p_w_pos)
        else:  # Token is absent from document
...
print("vii) acc, prec, rec, f1 ->", tuple(round(x*100,4) for x in (acc, prec, rec, f1)))
print("vii) confusion matrix (tp, tn, fp, fn) ->", (tp, tn, fp, fn))
