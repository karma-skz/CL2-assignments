...
def predict(text, model):
    ...
    V = len(model['vocab']) + 1  # Add 1 for UNK
    ...
    for w in words:
        if w in model['vocab']:
            count_neg = neg_wc.get(w, 0) # if we use neg_wc[w], it will give KeyError for unseen words
            count_pos = pos_wc.get(w, 0)
        else:
            # Unseen words -> use count = 0, laplace gives non-zero prob via V including UNK
            count_neg = 0
            count_pos = 0
        
        p_w_neg = (count_neg + 1) / (neg_total + V)
        p_w_pos = (count_pos + 1) / (pos_total + V)
...
print("ii) acc, prec, rec, f1 ->", tuple(round(x*100,4) for x in (acc, prec, rec, f1)))
print("ii) confusion matrix (tp, tn, fp, fn) ->", (tp, tn, fp, fn))
